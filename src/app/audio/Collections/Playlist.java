...
public final class Playlist extends AudioCollection {
    ...
    /**
     * Computes the total number of likes of the playlist
     */
    public void computeTotalLikes() {
        setTotalLikes(0);
        if (getSongs() != null) {
            ArrayList<Song> songsPlaylist = getSongs();
            for (Song song : songsPlaylist) {
                Integer songLikes = song.getLikes();
                setTotalLikes(getTotalLikes() + songLikes);
            }
        }
    }
}
