...
public final class Player {
     ...
     * @return the stats
     */
    public PlayerStats getStats(final String connection) {
        String filename = "";
        int duration = 0;
        if (source != null && source.getAudioFile() != null) {
        ...
        } else if (connection.equals("offline")) {
            // in case user is offline when status command is called,
            // remaining time of the player does not get updated
            return new PlayerStats(filename, duration, repeatMode, shuffle, paused);
        }
